...
class Pages extends DB
{
    ...
    {
        $db = $this::getInstance();
        // $query = $db->pdo->query("SELECT * FROM " . $this->table . " WHERE controller_page = 'Page' AND action_page = 'index'");
        $query = $db->pdo->query("SELECT * FROM " . $this->table);
    ...
    {
        $db = $this::getInstance();
        $query = $db->pdo->prepare("SELECT * FROM " . $this->table . " WHERE url_page = :url_page");
        $query->execute([
            'url_page' => $slug
        ]);
        $result = $query->fetch();
    ...
    {
        $db = $this::getInstance();
        $query = $db->pdo->prepare("SELECT * FROM " . $this->table . " WHERE id = :id");
        $query->execute([
            'id' => $id
        ]);
        $result = $query->fetch();
